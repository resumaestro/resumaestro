...
async function rotateConfigToken(
  refreshToken: string,
): Promise<{ token: string; refresh_token: string; exp?: number }> {
  const data = await slackForm('tooling.tokens.rotate', {
...
async function patchStoreSecret(
  env: Env,
  name: string,
  value: string,
): Promise<void> {
  ...
  const listed: any = await (
    await fetch(`${base}?per_page=100`, { headers })
  ).json();
  if (!listed.success)
    throw new Error(`list secrets failed: ${JSON.stringify(listed.errors)}`);
  ...
  if (!patched.success)
    throw new Error(`patch ${name} failed: ${JSON.stringify(patched.errors)}`);
}
...
async function writeBackTokens(
  ...
  if (!env.CF_API_TOKEN || !env.CF_ACCOUNT_ID || !env.SECRETS_STORE_ID) {
    return {
      persisted: false,
      detail:
        'write-back not configured (need CF_API_TOKEN, CF_ACCOUNT_ID, SECRETS_STORE_ID)',
    };
  }
  try {
...
async function updateManifest(req: Request, env: Env): Promise<Response> {
  ...
  if (
    !rawManifest ||
    (typeof rawManifest === 'object' && Object.keys(rawManifest).length === 0)
  ) {
  ...
  if (!appId)
    return j(
      { error: 'missing app_id (body.app_id or SLACK_APP_ID var)' },
      400,
    );
  ...
  try {
    ...
    let result = await slackForm(
      'apps.manifest.update',
      { app_id: appId, manifest },
      accessToken,
    );
    let rotated: { token: string; refresh_token: string; exp?: number } | null =
      null;
    ...
    const out: any = {
      ...
      app_id: appId,
      slack: ok
        ? { permissions_updated: result.permissions_updated ?? false }
        : result,
    ...
    return j(
      { error: 'update-manifest failed', detail: String(e?.message || e) },
      500,
    );
  }
}
...
export async function handleAdmin(
  ...
  if (!bearerOk(request, env)) return j({ error: 'unauthorized' }, 401);
  const path = new URL(request.url).pathname.replace(/^\/+/, ''); // e.g. "admin/update-manifest"
  switch (path) {
    case 'admin/update-manifest':
      return updateManifest(request, env);
    default:
