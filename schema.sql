...
-- dedupe re-adds, list "what's staged / parked", and render the /jobs board.
...
CREATE TABLE IF NOT EXISTS jobs (
  ...
  company         TEXT,                         -- denormalized for fast /jobs render
  role            TEXT,
  location        TEXT,
  ...
  status          TEXT NOT NULL DEFAULT 'scoring',
                  -- scoring | scored | researching | researched | tailoring | tailored | staged | parked
  research_level  TEXT NOT NULL DEFAULT 'none', -- none | surface | deep
  research_facets TEXT,                         -- JSON array (deep only) + free-form note
